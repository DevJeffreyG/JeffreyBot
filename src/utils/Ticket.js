...
class Ticket {
    ...
    async #createTicket(){
        ...
        if(topic === "help") {
            ...
            if(!warn.madeTicket) selectMenuWarn.addOptions({label, value: warn.id.toString(), description: Reglas[warn.rule_id].description});
            }
    ...
    async #resolveTicket(){
        ...
        let confirmation = await Confirmation("Marcar como resuelto", [`¿Estás segur@ de que quieres cerrar el ticket?`, `Sólo podría ser vuelto a abrir por un miembro del STAFF`], interaction);
        if(!confirmation) return;
